...
var agence_code = '';
var agent_name = '';
var parentAttributName = '';
var parentAttributValue = '';
...
(function ($) {
  ...
  $(document).on('change', '#agent-code', function (e) {
    ...
    window.location.href = APP_URL + "/agents?agent_name=".concat(agence_code);
  ...
  window.getColumns = function (object) {
    ...
    if (object.filterTree && object.filterTree.rows) {
      data = _objectSpread({}, data, {
        'rowFilter': object.filterTree.rows
      });
    }

    if (object.filterTree) {
      data = _objectSpread({}, data, {
        'dates': object.filterTree.dates
      });
    }
      ...
      success: function success(response) {
        if (object.filterElement) {
          ...
          if (response.rows && response.rows.length) {
            var rowsFilterData = response.rows.map(function (d, index) {
              return {
                id: d,
                text: d
              };
            ...
            new Tree(object.filterElement.rows, {
              data: [{
                id: '-1',
                text: response.rowsFilterHeader,
                children: rowsFilterData
              }],
              closeDepth: 1,
              loaded: function loaded() {
                if (response.filter && response.filter.rows_filter) {
                  this.values = object.filterTree.rows = response.filter.rows_filter; // console.log(this.values);
                }
              },
              onChange: function onChange() {
                object.filterTree.rows = this.values; // console.log(this.values);
              }
            });
          }
        ...
        if (response.columns.length) {
          var reformattedColumns = _toConsumableArray(response.columns).map(function (column) {
              ...
              render: function render(data, type, full, meta) {
                ...
                if (newData !== null && newData !== undefined) {
                  ...
                  if (newData.indexOf('|') !== -1) {
                    newData = newData.split('|').join('<br/>'); // newData = newData[0] + '<br/>' + newData[1];
                  }
                } else {
                  newData = '';
                }
          ...
          object.columns = _toConsumableArray(reformattedColumns);
        } else {
          object.columns = [{
            title: 'Résultats'
          }];
          ...
          try {
            ...
            if (params.details) {
              $('#' + object.element).on('click', 'td.details-control', function () {
                if (object.element === 'globalViewTable') {
                  var _row = object.element_dt.cell(this).index().row + 1;

                  var _values = $(tableId + " > tbody > tr:nth-child(" + _row + ") td:" + (params.details ? "nth-child(2)" : "first-child")).text().split(' / ');
                ...
                if (row.child.isShown()) {
                  ...
                  if (object.childneedParentValue && object.childneedParentValue === true) {
                    var _row2 = object.element_dt.cell(this).index().row + 1;

                    parentAttributName = object.rowName;
                    parentAttributValue = $('#' + object.element + " > tbody > tr:nth-child(" + _row2 + ") td:" + (params.details ? "nth-child(2)" : "first-child")).text();
                  }

                  data = _objectSpread({}, data, {
                    key_groupement: tr.find('td:nth-child(2)').text()
                  });
            ...
            $(tableId + ' tbody').on('click', 'td', function () {
              if (!$(this).hasClass('details-control')) {
                ...
                var col = object.element_dt.cell(this).index().column + 1;
                var row = object.element_dt.cell(this).index().row + 1;
                var colText = $(tableId + " > thead > tr > th:nth-child(" + col + ")").text();
                ...
                var rowText = $(tableId + " > tbody > tr:nth-child(" + row + ") td:" + (params.details ? "nth-child(2)" : "first-child")).text();
  ...
  window.InitDataTable = function (object) {
    var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    var params = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {
      removeTotal: true,
      removeTotalColumn: false,
    ...
    if ($.fn.DataTable.isDataTable(table)) {
      ...
      var tableID = object.element;
      var tableParent = table.parents('.card-body');
      table.remove();
      $('#' + tableID + '_wrapper').remove();
      var newTable = object.columns.reduce(function (accumulator, current) {
        return accumulator + '<th>' + current.title + '</th>';
      }, params.details ? '<th></th>' : '');
      newTable = '<table id="' + tableID + '" class="table table-bordered table-striped table-valign-middle capitalize">' + '<thead>' + newTable + '</thead><tbody></tbody></table>';
      tableParent.append(newTable);
      table = $('#' + object.element);
    }

    if (params.details) {
      object.objDetail.columns = _toConsumableArray(object.columns);
      object.objDetail.columns = object.objDetail.columns.map(function (item, index) {
        if (index === 0) {
          return _objectSpread({}, item, {
            data: 'Resultat_Appel',
            name: 'Resultat_Appel',
            title: 'Resultat Appel'
          });
        }
  ...
  window.InitChart = function (objectChart, columns, data) {
    ...
    labels = labels.map(function (column) {
      return column.data;
    });
    ...
    datasets = datasets.map(function (item) {
      var regions = item.values.map(function (value) {
        return parseFloat(isNaN(value) ? value.replace('%', '') : value);
      });
      var _dataItem = {
        label: item[column],
        backgroundColor: dynamicColors(uniqueColors),
        data: regions
    ...
    var chartID = objectChart.element_id;
    var chart = $('#' + chartID);

    if (!detailClick) {
      var chartParent = chart.parents('.col-12');
      chartParent.children('.chartjs-size-monitor').remove();
      chart.remove();
      var newChart = '<canvas id="' + chartID + '">';
  ...
  window.getDatesFilter = function (globalElements) {
    $.ajax({
      ...
      method: 'GET',
      success: function success(response) {
        var treeData = response.dates;
        $('.tree-view').each(function (index, item) {
          var treeId = '#' + $(this).attr('id');
          var object = globalElements.filter(function (element) {
  ...
  window.userFilter = function (userObject) {
    var isPost = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    $.ajax({
      url: APP_URL + '/user/filter',
      method: isPost ? 'POST' : 'GET',
      data: {
        filter: userObject.filterTree.dates
      },
      success: function success(response) {
  ...
  window.filterSelectOnChange = function (caller, agence_code, agent_name) {
    var agenceParam = agence_code ? 'agence_code=' + agence_code : '';
    var agentParam = agent_name ? 'agent_name=' + agent_name : '';
    var params = agenceParam || agentParam ? '?' + agenceParam + '&' + agentParam : '';
    var url = APP_URL + '/' + $(caller).val() + params;
  ...
  window.checkDataCount = function () {
    var ImportingReady = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
    $('.import_status-wrapper').removeClass('d-none');
    var sendRequestCountData = Boolean(JSON.parse(window.localStorage.getItem('sendRequestCountData')));

    if (sendRequestCountData) {
      var importedDataElement = $('.imported-data');
      var totalImportedData = window.localStorage.getItem('totalImportedData');

      if (importedDataElement.length && ImportingReady) {
        importedDataElement.text(totalImportedData + ' lignes inserées');
      }

      var request_resolved = Boolean(JSON.parse(window.localStorage.getItem('request_resolved')));
      $.ajax({
        method: 'get',
        ...
        success: function success(data) {
          if (data.flags) {
            ...
            if (request_resolved || isImporting == 2) {
              window.localStorage.removeItem('sendRequestCountData'); // window.localStorage.removeItem('totalImportedData');
  ...
  window.createChild = function (row, object) {
    var data = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
    detailClick = true; // This is the table we'll convert into a DataTable

    var objectChild = object.objDetail;
    ...
    var objectChildItem = _objectSpread({}, objectChild);

    object.children.push(objectChildItem);
    var createdChild = tableDom; // Display it the child row

    row.child($(tableDom)).show();
    var table = $('#' + objectChildItem.element);
    table.after(canvasDom); // row.child(objectChild.element).show();

    data = _objectSpread({}, data, {
      'route': object.routeData
    });
  ...
  window.destroyChild = function (row) {
    ...
    table.detach();
    table.DataTable().destroy(); // And then hide the row

    row.child.hide();
  };

  window.toggleLoader = function (parent) {
  ...
  window.elementExists = function (object) {
    if (object !== null && object !== undefined) {
      var element = object instanceof jQuery ? object : $('#' + object.element);
  ...
  window.feedBack = function (message, status) {
    swal(status.replace(/^\w/, function (c) {
...
/***/ "./resources/sass/custom/_pages/_dashboard.sass":
/*!******************************************************!*\
  !*** ./resources/sass/custom/_pages/_dashboard.sass ***!
  \******************************************************/
/*! no static exports found */
...
/***/ 0:
/*!*****************************************************************************************************************************************************************************************************************************************************************************************************************************!*\
  !*** multi ./resources/js/app.js ./resources/sass/main.scss ./resources/sass/oneui/themes/amethyst.scss ./resources/sass/oneui/themes/city.scss ./resources/sass/oneui/themes/flat.scss ./resources/sass/oneui/themes/modern.scss ./resources/sass/oneui/themes/smooth.scss ./resources/sass/custom/_pages/_dashboard.sass ***!
  \*****************************************************************************************************************************************************************************************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
