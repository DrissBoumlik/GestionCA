$(function () {
    ...
    let statsProcessingDelay = {
        ...
        data: undefined,
        filterTree: {dates: [], rows: [], datesTreeObject: undefined},
        filterElement: {dates: '#tree-view-09', rows: ''},
        filterQuery: {
            appCltquery: true,
        },
    ...
    let globalElements = [userObject, statsProcessingDelay];
    ...
    $("#printElement").on("click", function () {
        ...
        setTimeout(function () {
            let statsProcessingDelayChart = document.getElementById('statsProcessingDelayChart');
            ...
            //creates image
            let statsProcessingDelayChartImg = statsProcessingDelayChart.toDataURL("image/png", 1.0);
            let doc = new jsPDF('p', 'pt', [842, 842]);
            doc.addImage(logo, 'jpeg', 371, 10, 100, 30);
            doc.text(10, 40, 'Délai de traitement BF5 et BF8');
            doc.autoTable({html: '#statsProcessingDelay', pageBreak: 'auto', margin: {top: 50}, styles: {fontSize: 7}});
            doc.addImage(statsProcessingDelayChartImg, 'JPEG', 150, 400, 500, 350);
            doc.addImage(footer, 'jpeg', 371, 810, 100, 30);
            doc.save('Traitement Assistance.pdf');
