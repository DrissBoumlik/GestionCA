$(function () {
    ...
    $("#printElement").on("click", function () {
        ...
        doc.text(5, 20, 'Résultats Appels');
        doc.autoTable({html: '#statsRegions', margin: {left: 5,top: 60}, pageBreak: 'auto',tableWidth: 195});
        doc.addImage(statsRegionsChartImg, 'JPEG', 200, 30, 80, 100);
        doc.addPage();
        doc.text(5, 20, 'Répartition des dossiers traités par périmètre');
        doc.autoTable({html: '#statsFolders', margin: {left: 5,top: 60}, pageBreak: 'auto',tableWidth: 195});
        doc.addImage(statsFoldersChartImg, 'JPEG', 200, 30, 80, 100);
        doc.addPage();
        doc.text(5, 20, 'Résultats Appels Préalables par agence');
        doc.autoTable({html: '#callsStatesAgencies', margin: {left: 5,top: 60}, pageBreak: 'auto',tableWidth: 195,columnStyles: { 0: { cellWidth : 20 } }});
        doc.addImage(callsStatesAgenciesChartImg, 'JPEG', 200, 30, 80, 100);
        doc.addPage();
        doc.text(10, 20, 'Résultats Appels Préalables par semaine');
        doc.autoTable({html: '#callsStatesWeeks', margin: {left: 5,top: 60}, pageBreak: 'auto',tableWidth: 195});
        doc.addImage(callsStatesWeeksChartImg, 'JPEG', 200, 30, 80, 100);
        doc.addPage();
        doc.text(10, 20, 'Code Interventions liés aux RDV Confirmés (Clients Joignables)');
        doc.autoTable({html: '#statsCallsPos', margin: {left: 5,top: 30}, pageBreak: 'auto',tableWidth: 195,columnStyles: { 0: { cellWidth : 20 } }});
        doc.addImage(statsCallsPosChartImg, 'JPEG', 200, 30, 80, 100);
        doc.addPage();
        doc.text(10, 20, 'Code Interventions liés aux RDV Non Confirmés (Clients Injoignables)');
        doc.autoTable({html: '#statsCallsNeg', margin: {left: 5,top: 30}, pageBreak: 'auto',tableWidth: 195,columnStyles: { 0: { cellWidth : 20 } }});
        doc.addImage(statscallsNegChartImg, 'JPEG', 200, 30, 80, 100);
        doc.addPage();
        doc.text(10, 10, 'Répartition des dossiers non validés par Code Type intervention');
        doc.addImage(statsFoldersByTypeChartImg, 'JPEG', 200, 30, 80, 100);
        doc.autoTable({html: '#statsFoldersByType', margin: {left: 5,top: 15}, pageBreak: 'auto',tableWidth: 195 });
        doc.addPage();
        doc.text(10, 20, 'Répartition des dossiers non validés par code intervention');
        doc.autoTable({html: '#statsFoldersByCode', margin: {left: 5,top: 30}, pageBreak: 'auto',tableWidth: 195 });
        doc.addImage(statsFoldersByCodeChartImg, 'JPEG', 200, 30, 80, 100);
        doc.addPage();
        doc.text(10, 20, 'Production Globale CAM');
        doc.autoTable({html: '#statsPerimeters', margin: {left: 5,top: 30}, pageBreak: 'auto',tableWidth: 195});
        doc.addImage(statsPerimetersChartImg, 'JPEG', 200, 30, 80, 100);
