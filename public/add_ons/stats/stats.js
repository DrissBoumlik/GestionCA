$(function () {
                    ...
                    try {
                        ...
                        if (params.details) {
                            $('#' + object.element).on('click', 'td.details-control', function () {
                                const tr = detailstr = $(this).closest('tr');
                                const row = detailsrow = object.element_dt.row(tr);
    ...
    $("#printElement").on("click", function () {
        ...
        if (detailsrow.child.isShown()) {
            // This row is already open - close it
            destroyChild(detailsrow);
            detailstr.removeClass('shown');
        }
        ...
        doc.autoTable({html: '#statsRegions', margin: {left: 0 , top: 50}, pageBreak: 'auto', tableWidth: 520,styles: {fontSize: 7} });
        doc.addImage(statsRegionsChartImg, 'JPEG', 532 , 30 , 350 , 300);
        doc.text(10, 390, 'Répartition des dossiers traités par périmètre');
        doc.autoTable({html: '#statsFolders', pageBreak: 'auto', tableWidth: 520, startY: 400, margin: {left: 0} ,styles: {fontSize: 7} });
        doc.addImage(statsFoldersChartImg, 'JPEG', 532, 400  , 350 , 350);
        doc.addPage();
        doc.text(10, 20, 'Résultats Appels Préalables par agence');
        doc.autoTable({html: '#callsStatesAgencies', margin: {left: 0 , top: 30}, pageBreak: 'auto', tableWidth: 520 , styles: {fontSize: 7}  });
        ...
        doc.autoTable({html: '#statsCallsPos', margin: {left: 0, top: 30}, pageBreak: 'auto', tableWidth :842,  styles: {fontSize: 7} });
        ...
        doc.autoTable({html: '#statsCallsNeg', margin: {left: 0, top: 30}, pageBreak: 'auto',tableWidth :842 ,styles: {fontSize: 7}});
        ...
        doc.autoTable({html: '#statsFoldersByType', margin: {left: 0 , top: 30}, pageBreak: 'auto',tableWidth: 525,styles: {fontSize: 7} });
        doc.addPage();
        ...
        doc.autoTable({html: '#statsFoldersByCode', margin: {left: 0 , top: 30}, pageBreak: 'auto',tableWidth: 525,styles: {fontSize: 7,cellPadding: {top: 3, bottom: 3}} });
        doc.addPage();
        doc.text(10, 20, 'Production Globale CAM');
        doc.autoTable({html: '#statsPerimeters', margin: {left: 0 , top: 30}, pageBreak: 'auto', tableWidth: 520,styles: {fontSize: 7}});
